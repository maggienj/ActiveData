...
def to_painless(self, not_null=False, boolean=False, many=False):
    ...
    if (not test_v or test_v.to_painless(boolean=True) == "false") and not test_l:
        expr = v + ".substring(0, max(0, min(" + v + ".length()) " + "\"params\":" + "{\"length\":" + l
    else:
        expr = "((" + test_v.to_painless(boolean=True) + ") || (" + test_l.to_painless(
            boolean=True) + ")) ? null : (" + v + ".substring(0, max(0, min(" + v + ".length()) " + "\"params\":" + "{\"length\":" + l
    return expr
...
def to_painless(self, not_null=False, boolean=False, many=False):
    ...
    expr = "((" + test_v + ") || (" + test_l + ")) ? null : (" + v + ".substring(min(" + v + ".length(), max(0, (" + v + ").length()) " + "\"params\":" + "{\"length\":" + l
    return expr
...
def to_painless(self, not_null=False, boolean=False, many=False):
    ...
    expr = "((" + test_v + ") || (" + test_l + ")) ? null : (" + v + ".substring(0, min(" + v + ".length(), max(0, (" + v + ").length()) " + "\"params\":" + "{\"length\":" + l
    return expr
...
def to_painless(self, not_null=False, boolean=False, many=False):
    ...
    if (not test_v or test_v.to_painless(boolean=True) == "false") and not test_l:
        ...
        expr = "((" + test_v.to_painless(boolean=True) + ") || (" + test_l.to_painless(boolean=True) + ")) ? null : (" + v + ".substring(0, max(0, min(" + v + ".length()) " + "\"params\":" + "{\"length\":" + l
    return expr
